...
class ModelEMA:
    ...
    def __init__(self, model, decay=0.9999, tau=2000, updates=0):
        ...
        # self.ema = deepcopy(de_parallel(model).cpu()).eval().to(self.device)
        # model_de_parallel = de_parallel(model)
        # device = next(model_de_parallel.parameters()).device  # Get device from original model
        # self.ema = model_de_parallel.__class__(model_de_parallel.yaml).to(device)  # Move to same device
        # self.ema.load_state_dict(model_de_parallel.state_dict())
        # self.ema.eval()
