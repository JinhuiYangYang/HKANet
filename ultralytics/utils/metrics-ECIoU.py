...
def bbox_iou(box1, box2, x1y1x2y2=True, GIoU=False, DIoU=False, CIoU=False, ECIoU=False, eps=1e-7):
    ...
    iou = inter / union
    if GIoU or DIoU or CIoU or ECIoU:
        cw = torch.max(b1_x2, b2_x2) - torch.min(b1_x1, b2_x1)  # convex (smallest enclosing box) width
        ch = torch.max(b1_y2, b2_y2) - torch.min(b1_y1, b2_y1)  # convex height
        if CIoU or DIoU or ECIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            if DIoU:
                ...
                v = (4 / math.pi ** 2) * torch.pow(torch.atan(w2 / h2) - torch.atan(w1 / h1), 2)
                alpha = v / (v - iou + (1 + eps))
                return iou - (rho2 / c2 + rho_w2 / cw2 + rho_h2 / ch2 + v * alpha)  # EIoU with CIoU alpha*v
