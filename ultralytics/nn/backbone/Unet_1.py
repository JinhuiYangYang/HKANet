...
class UNet(nn.Module):
    def __init__(self, in_c=3, dims = [64, 128, 256, 512, 1024]):
        ...
        self.stem = nn.Sequential(
            nn.Conv2d(in_c, self.dims[0], kernel_size=1, bias=False),
            nn.GroupNorm(num_groups=1, num_channels=self.dims[0]),
        ...
        # self.e6 = DWBlocks(self.dims[5], 2)     
...
class DownSample(nn.Module):
    ...
    def forward(self, x):
        ...
        x3 = self.mlp(torch.cat([x1, x2], dim=1))
        return x3
